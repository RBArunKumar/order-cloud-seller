...
const OrderCard: FunctionComponent<OrderCardProps> = ({ order }) => {
  ...
  const dateCreated: string = order.DateCreated !== undefined ? order.DateCreated : '';
  ...
  return (
    <Card variant="">
      <VStack w="full" width="100%" justifyContent="space-between" p={2}>
        ...
        >
          <Text>
            <b>Order Date : </b> {formatShortDate(dateCreated)}
          </Text>
          <Text>
          ...
          {!paidWithPoints && (
            <Text>
              <b>Order Total : </b> {formatPrice(orderTotal)}
            </Text>
          )}
          {paidWithPoints && (
            <Text>
