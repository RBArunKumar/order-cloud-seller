import { queryScopes, serializeCH1RichText } from './data.js';
...
export function setDOMAttribute(element, attribute, value, config) {
    ...
    if (attribute == 'data-path-src') {
        ...
        const picture = Array.from(element.children).find((n) => n.localName == 'picture') || element;
        target = picture.querySelector('img, picture');
    }
    ...
    if (!target)
        return;
    var placeholder = String((_b = (element.getAttribute('data-path-placeholder') || target[prop])) !== null && _b !== void 0 ? _b : target.getAttribute(prop));
    if (value != null) {
        if (prop == 'innerHTML') {
